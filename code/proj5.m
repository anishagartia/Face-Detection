...
%  + get_positive_features.m  <--- You code this -- done
...
% set use_vl_hog to false to generate HOG features using own code instead
% of vl_hog
...
%% LFW Dataset
if (use_lfw_dataset == true)
    tic
    lfw_folder_crop_path = '../data/lfw_crop';
    features_lfw = get_positive_features( lfw_folder_crop_path, feature_params );
    features_pos = [features_pos; features_lfw];
    toc
end
...
if (use_lfw_dataset == true)
    lambda = 0.001;        
elseif (use_vl_hog == false)
    lambda = 0.0006;
elseif (use_hard_negative_mining == true)
    lambda = 0.0001;
end
...
features_neg_orig = features_neg;
...
if (use_hard_negative_mining == true)
    tic
    fprintf('Detecting Hard Negative Features\n');
    ...
    %0.005 = 71.1;
    %0.01 = 74.8   
    [hard_mining_feats] = hard_mining(non_face_scn_path, w, b, feature_params,threshold1);
    
    % train new SVM

    features_neg = [features_neg_orig; hard_mining_feats];
    X = [features_pos ; features_neg];
    Y = [ones(size(features_pos,1),1); -1*ones(size(features_neg,1),1)];
    [w, b] = vl_svmtrain(X', Y, lambda2); 
    
    fprintf('After Hard Mining: Classifier performance on train data:\n')
    confidences = [features_pos; features_neg]*w + b;
    label_vector = [ones(size(features_pos,1),1); -1*ones(size(features_neg,1),1)];
    [tp_rate, fp_rate, tn_rate, fn_rate] =  report_accuracy( confidences, label_vector );
...
%% Hardmining: LFW, A. Gudimella

% if (use_hard_negative_mining == true)
%     image_files = dir( fullfile( extra_non_face_scn_path, '*.jpg' ));
%     num_images = length(image_files);
%     extra_features_neg = zeros(num_images, ...
%         (feature_params.template_size / feature_params.hog_cell_size)^2 * 31);
%     fprintf(1, '      ');
%     for i = 1:num_images
%         fprintf(1, '\b\b\b\b\b\b%6.2f', i*100/num_images);
%         img = single(imresize(imread(fullfile(extra_non_face_scn_path,image_files(i).name)), [36 36]));
%         feat = vl_hog(img, feature_params.hog_cell_size);
%         extra_features_neg(i,:) = reshape(feat, 1, []); 
%     end
%     features_neg = [features_neg; extra_features_neg];
...
if (use_neural_net == true)
    [bboxes, confidences, image_ids] = run_detector_nnet(test_scn_path, w, b, feature_params);
    f_nnlables = fopen('nn_lables.txt');
    nn_lables = cell2mat(textscan(f_nnlables,'%d'));
    I = find(nn_lables);
    bboxes = bboxes(I,:);
    confidences = confidences(I);
    image_ids = image_ids(I,:);
elseif (use_vl_hog == true)
