...
function features_pos = get_positive_features2(train_path_pos, feature_params)
...
D = (feature_params.template_size / feature_params.hog_cell_size)^2 * 9;
features_pos = zeros(num_images,D);
fprintf('Getting positive features: image       ');
for i = 1:num_images
    fprintf('\b\b\b\b\b\b%6.0f',i);
    this_feat = get_hog_feat(im2single(imread(fullfile(train_path_pos,char(image_files(i).name)))), feature_params.hog_cell_size);
    features_pos(i,:) = im2double(reshape(this_feat,1,D));
end
